...
int main() {
  ...
  std::cout << "Rozmiar kolejki: " << triangleQueue.size() << std::endl;

  if (!triangleQueue.empty()) {
    std::cout << "Pierwszy element kolejki: ";
    triangleQueue.front().Drukuj();
  }

  if (!triangleQueue.empty()) {
    std::cout << "Ostatni element kolejki: ";
    triangleQueue.back().Drukuj();
  }

  triangleQueue.pop();

  std::cout << "Po usunięciu jednego elementu, rozmiar kolejki: " << triangleQueue.size() << std::endl;
