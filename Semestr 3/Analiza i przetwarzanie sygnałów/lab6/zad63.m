...
for m = 1:Many % petla analizy
    ...
    % Dodaj rysunek w k-tej iteracji pętli głównej
    figure;
    semilogy(f, X2); grid; title(['PSD Welcha - Iteracja ', num2str(m)]); xlabel('f [Hz]'); ylabel('V^2 / Hz');
    pause
end
...
% Porównanie z funkcją pwelch w Matlabie
[Pxx, F] = pwelch(x, hamming(Mwind), Mwind-Mstep, Mfft, fpr);
figure;
semilogy(F, Pxx); grid; title('PSD pwelch'); xlabel('f [Hz]'); ylabel('V^2 / Hz');
