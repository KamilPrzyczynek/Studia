...
% Alternatywna definicja impulsowa filtra
...
% Weryfikacja odpowiedzi czestotliwosciowej filtra: amplitudowej i fazowej
fs = 2000;
f = 0:1:fs/2; % czestotliwosc probkowania [Hz], wybrane czestotliwosci
...
figure;
plot(f, angle(exp(j * 2 * pi * f / fs * M) .* H));
xlabel('f [Hz]');
title('angle(H(f)) [rad]');
grid;
